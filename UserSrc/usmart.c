...
u8 usmart_get_fname(u8*str,u8*fname,u8 *pnum)
{
  ...
  while(1)
  {
    if(*strtemp==0)
    {
      ...
      break;
    }else if(*strtemp=='(')fover++;//括号深度增加一级	 
    else if(*strtemp==')')//
    {
      if(fover)fover--;
    ...
    {
      if(*strtemp==',')temp=1;		//使能增加一个参数
      else if(*strtemp!=' ')temp++;	//得到有效参数(非空格)
      if(fover==1&&temp==2)
      {
        temp++;		//防止重复增加
        parmnum++; 	//参数增加一个
...
void usmart_exe(void)
{
  ...
  for(i=0;i<pnum;i++)//输出参数
  {
    ...
    {
      temp[i]=*(u32*)(usmart_dev.parm+usmart_get_parmpos(i));
      if(usmart_dev.sptype==SP_TYPE_DEC)printf("%lu",(unsigned long)temp[i]);//10进制参数显示
...
void usmart_scan(void)
{
  ...
  {					   
    ...
    for(int foo=0;foo<USART_REC_LEN;foo++)
      USART_RX_BUF[foo]=0;
  }
  else
  {
    if(*uart1_buff == 0x0d || *uart1_buff == 0x0a)
      USART_RX_STA|=0x8000;	//接收完成了
    else
    {
      USART_RX_BUF[USART_RX_STA&0X3FFF] = *uart1_buff ;
      USART_RX_STA++;
